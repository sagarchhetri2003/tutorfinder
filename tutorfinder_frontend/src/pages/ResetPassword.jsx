...
//     try {
//       const res = await axios.post("http://localhost:5500/api/user/reset-password", {
//         token,
...
const ResetPassword = () => {
  ...
  const handleReset = async (e) => {
    ...
    try {
      const res = await axios.post(`http://localhost:5500/api/user/reset-password/${token}`, {
        newPassword: password,
      });
      ...
      // Clear any reset-related flags if used
      localStorage.removeItem("forgotEmail");
      localStorage.removeItem("otpVerified");
